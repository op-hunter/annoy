...
public:
    ...
    virtual bool delete_vec_by_id(const S item, char** error=NULL) = 0;
...
public:
    ...
    void show_info() {
        std::cout << "main member of annoy index: " << std::endl;
        std::cout << "dim: " << _f << std::endl;
        std::cout << "size of each node: " << _s << std::endl;
        std::cout << "num of vectors: " << _n_items << std::endl;
        std::cout << "num of nodes: " << _n_nodes << std::endl;
        std::cout << "size of nodes array: " << _nodes_size << std::endl;
        std::cout << "num of child: " << _K << std::endl;
        std::cout << "is loaded: " << _loaded << std::endl;
        std::cout << "is verbose: " << _verbose << std::endl;
        std::cout << "fd: " << _fd << std::endl;
        std::cout << "is on disk: " << _on_disk << std::endl;
        std::cout << "is built: " << _built << std::endl;
    }
    ...
    bool delete_vec_by_id(const S item, char** error=NULL) {
        std::cout << "enter delete by id, item: " << item << std::endl;
        if (item > _n_items || item < 0) {
            set_error_from_string(error, "invalid id");
            return false;
        }
        Node* x = _get(item);
        ...
        while (nns.size() < search_k && !q.empty()) {
            ...
            if (nd->n_descendants == 1 && i < _n_items) {//raw data
                ...
                for (auto ii = 0; ii < nd->n_descendants; ++ ii) {
                    Node* tmp_nd = _get(dst[ii]);
                    if (tmp_nd->n_descendants == 1)
